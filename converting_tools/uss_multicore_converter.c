...
int main(void)
{
	...
	FILE *fp = fopen("to_dCollideSpheres3.txt", "r");
	FILE *fp1 = fopen("to_dCollideSpheres3Hex.txt", "w");
	FILE *fp2 = fopen("outputs.txt", "r");
	FILE *fp3 = fopen("from_dCollideSpheres33.txt", "w");
	...
	while(fgets(nString, 16, fp) != NULL)
	{
		...
		if(count == 32)
		{
			fprintf(fp1, "push 32 0x00000000\n");	
			fprintf(fp1, "push 32 0x00000001\n");	

			int j;
			for(j = 33; j < 73; j = j + 1)
			{
				if(j == 42 || j == 52 || j == 62 || j == 72)
				{
					fprintf(fp1, "popfile %d\n", j);	
				}
				else
				{
					fprintf(fp1, "pop %d\n", j);	
