                          ...
                          dContactGeom *contact, int skip)
{
	...
	float r1 = (float)sphere1->radius;
	float r2 = (float)sphere1->radius;

	float x1 = (float) o1->final_posr->pos[0];
	float y1 = (float) o1->final_posr->pos[1];
	float z1 = (float) o1->final_posr->pos[2];

	float x2 = (float) o2->final_posr->pos[0];
	float y2 = (float) o2->final_posr->pos[1];
	float z2 = (float) o2->final_posr->pos[2];
	
	float cX = (float) contact->pos[0];
	float cY = (float) contact->pos[1];
	float cZ = (float) contact->pos[2];
	
	float nX = (float) contact->normal[0];
	float nY = (float) contact->normal[1];
	float nZ = (float) contact->normal[2];

	float depth = (float) contact->depth;

	FILE *fp = fopen("to_dCollideSpheres2.txt", "a");
...
/*
	...
	fprintf(fp, "Contact\n");
	fprintf(fp, "--------------\n");
	fprintf(fp, "ColXYZ: %f, %f, %f\n", cX, cY, cZ);
	fprintf(fp, "NorXYZ: %f, %f, %f\n", nX, nY, nZ);
	fprintf(fp, "Depth : %f\n", depth);
*/
	//fprintf(fp, "====================================\n");
