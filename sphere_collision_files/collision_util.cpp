...
float readLine(char *file, int offset, int first)
{
        int size = 0;
	...
	char line[9] = {'0','0','0','0','0','0','0','0','\0'};
        ...
        int i;
        for(i = offset; i < strlen(file); i = i + 1)
        {
                if(file[i] == '\n')
                {
                        break;
	...
	printf("%d\n", offset);
        ...
        if(first == 1)
        {
                if(atoi(line) == 0)
                {
                        //return atoi(line) + size + 1;
			temporary = atoi(line) + size + 1;
                }
		else
		{
			temporary = atoi(line);
                     ...
                     dVector3 p2, dReal r2, dContactGeom *c)
{
    ...
    FILE *fp = fopen("from_dCollideSpheresNUMBERS.txt", "a");
//    FILE *fp = fopen("from_dCollideSpheres55.txt", "r+b");
//    FILE *fp = fopen("from_dCollideSpheres3.txt", "a");
	...
	return 1;
/*	
	float cX = (float) c->pos[0];
        float cY = (float) c->pos[1];
        float cZ = (float) c->pos[2];

        float nX = (float) c->normal[0];
        float nY = (float) c->normal[1];
        float nZ = (float) c->normal[2];

	float depth = (float) c->depth;
...
*****************************************************************
	//read from file code
	float off, cX, cY, cZ, nX, nY, nZ, depth;
	long fsize;
	...
	fflush(fp);
        ...
        off = readLine(whole, offset, first);
        offset = offset + temporary;
        first = 0;

        cX = readLine(whole, offset, first);
        offset = offset + temporary;

	cY = readLine(whole, offset, first);
       	offset = offset + temporary;

       	cZ = readLine(whole, offset, first);
       	offset = offset + temporary;

        nX = readLine(whole, offset, first);
       	offset = offset + temporary;

       	nY = readLine(whole, offset, first);
       	offset = offset + temporary;

       	nZ = readLine(whole, offset, first);
       	offset = offset + temporary;

       	depth = readLine(whole, offset, first);
       	offset = offset + temporary;
	...
	fflush(fp);
        fclose(fp);
	if(cX == 0 && cY == 0 && cZ == 0 && nX == 0 && nY == 0 && nZ == 0 && depth == 0)
	{
		return 0;
	}    
